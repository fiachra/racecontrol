...
class RunnerInfoHolder {
  ...
  async update() {
    ...
    this.studentsByClass = this.runners.reduce((a, v) => {}, {})
  }
    ...
    async.waterfall([
      function(innerCB) {
        ...
        that.updateStudentInfo(function(err) {
          if (err) { innerCB(err) } else { innerCB() }
        })
      },
      function(innerCB) {
        console.log('HERE2')
    ...
    Object.keys(grouped)
      .forEach(function(key, index) {
        ...
        if (foundStu.tags.length === 1) {
          ...
          foundStu.starttimestr = moment(foundStu.tags[0].time)
            .format('LTS')
          ...
          foundStu.starttimestr = moment(foundStu.tags[1].time)
            .format('LTS')
          foundStu.endtimestr = moment(foundStu.tags[0].time)
            .format('LTS')
        ...
        if (running === 5) {
          ...
          async.waterfall([
            ...
            function(ininCB) {
              ...
              if (raceTime > 1800) {
                var endTime = moment(moveTime)
                  .add(raceTime, 'seconds')
                dynamoFuncs.addCheckin(student.racenumber, 'fiachra', endTime.toISOString(), function(err) {
                  if (err) { ininCB(err) } else { ininCB() }
                })
              } else {
